...
> **Status:** 0.10.0 — adopts titan-tyr v0.11.0 surface: third contract subtype `connection` (structural binding declared in build/config/deploy artifacts — Dockerfiles, compose, k8s manifests, `.gitmodules`) with a `connection_type` sub-discriminator (`builds-from`, `instantiates`, `runs`, `member-of`, `depends-on`, `submodule`). The subtype chip gets a fifth color; a secondary `connection-type-chip` renders next to it on connection contracts in both ContractDetail and PartDetail. Templates browser auto-grows to 5 kinds (the new `/templates/connection` page works the same as the others — index, body, version history, "Used by"). Read-only as ever — register-contract flow stays out of MVP scope. Builds on 0.9.0 (graph focus). Companion contract proposal `2.1.0-rc1` on the mimiron↔tyr contract; accepted as 2.1.0 once this build is verified live.
