...
> **Status:** 0.7.0 — adds a Templates browser as a sibling page (`#/templates`, `#/templates/:kind`) with its own 2-pane layout (no graph, no resize handle) for the four titan-tyr templates: `software`, `container`, `interaction`, `binding`. Header gets a `Parts | Templates` nav with active-state. The `tpl <kind>@<ver>` chip in part/contract topbars is now a router-link to the matching template. Legacy stamps (e.g. `contract` from before the v0.10.0 rename to `interaction`) 404 cleanly with a "likely renamed" hint. Builds on 0.6.0 (titan-tyr v0.10.0 surface adoption + subtype chips). The four-view graph design (Software / DevOps / Interfaces tabs with full part-subtype rendering and environment switcher) from [DESIGN.md](./DESIGN.md) stays deferred; drift visualization (parts on outdated stamps) is the next templates-related follow-up.
