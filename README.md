...
> **Status:** 0.8.0 — adds a "Used by" panel to each template page (`#/templates/:kind`) that walks parts (or contracts, for interaction/binding) client-side, fetches each body to read its template stamp, and groups results into On active / Drifted (by version) / Unstamped buckets with a counts strip. Lazy on expand; per-body fetches concurrency-capped at 6. Closes the drift-visualization gap called out in 0.7.0. Builds on 0.7.0 (Templates browser) and 0.6.0 (titan-tyr v0.10.0 surface). The four-view graph design (Software / DevOps / Interfaces tabs with full part-subtype rendering and environment switcher) from [DESIGN.md](./DESIGN.md) stays deferred. If per-body fetches start hurting on larger catalogs, the follow-up is a server-side `GET /templates/{kind}/usage` aggregate (titan-tyr).
