...
> **Status:** 0.13.0 — graph view tabs (closes #5). Three-tab strip above the graph (All / Software / DevOps) replaces the deferred four-tab spec from DESIGN.md — the original Interfaces tab collapsed into a use-case the graph focus filter (#29) already covers. **Software** filters to software parts + interaction contracts (application architecture); **DevOps** filters to image/container/pod/compose parts + binding/connection contracts (deployment chain) with software endpoints carried in to keep cross-stage edges connected. Active tab persists in localStorage. Tab switch re-renders the SVG with the filtered subset and clears any active focus (since the focused subgraph may not exist in the new view). No new API calls — fetched data is cached after the first load. Builds on 0.12.0 (catalog sidebar grouping).
