...
> **Status:** 0.12.0 — catalog sidebar now grouped into collapsible sections by subtype (closes #7). Section order follows the source/runtime/contract chain: software → image → container → pod → compose → interaction → binding → connection. Empty sections are omitted (they appear automatically as the catalog grows). Each section header shows the subtype chip + entry count and toggles collapse on click; collapse state persists per-section in localStorage. The catalog now includes contracts as well as parts — contract rows show `owner → counterparty` with the version chip and connection_type chip when relevant, routing to `/contracts/:id`. Search applies across both parts and contracts client-side. Pagination is dropped in favor of grouping (load-everything via `fetchAll`); the load-more button is gone. Builds on 0.11.0 (titan-tyr v0.14.0 surface).
