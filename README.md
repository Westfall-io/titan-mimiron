...
> **Status:** 0.9.0 — graph navigation: clicking a node in the graph filters to that node + one-hop neighbors + connecting edges; clicking an edge filters to its two endpoints + that one edge. Clear-focus is a full reset (drops the filter, clears the route, removes the markdown body and node-selected highlight) and is reachable three ways: a `clear focus` link in the legend strip, the ESC key, or clicking the empty graph background. Filter is CSS `display:none` (no Mermaid re-render — preserves the #14 layout-stability win). Builds on 0.8.0 ("Used by" panel). Graph overflow / scrolling on larger catalogs (#28) and server-side template-usage aggregation are still open follow-ups.
