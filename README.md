...
> **Status:** 0.21.0 — speculative rendering for the K8s runtime subtypes coming in [titan-archaedas#9](https://github.com/Westfall-io/titan-archaedas/issues/9) / titan-tyr's new template work (closes [#62](https://github.com/Westfall-io/titan-mimiron/issues/62), [#63](https://github.com/Westfall-io/titan-mimiron/issues/63), [#64](https://github.com/Westfall-io/titan-mimiron/issues/64), [#65](https://github.com/Westfall-io/titan-mimiron/issues/65)). Graph gains tier slots and color palette entries for `deployment` / `statefulset` / `job` / `service` / `ingress` / `secret` / `configmap`, layered above the existing compose/build tiers via dagre's negative-rank pinning so existing graph positions don't shift. "DevOps" view tab renamed to **Runtime** and unions compose + K8s subtypes (WatcherVault is the coexistence case). Catalog grouping order extends with the K8s cluster placed after the compose-deploy cluster. `secret` parts gain a "🔒 sensitive — key names only" banner above the body as a self-enforcing convention. All four ship speculatively against the enum names locked in archaedas#9 — inert until tyr ships templates and archaedas registers parts.
>
> Builds on 0.20.1 (Cache-Control headers), 0.20.0 (init-container handoff shim), 0.19.0 (Cytoscape.js + dagre graph renderer), 0.18.2 (Software view shows all intra-software contracts), 0.18.1 (titan-mimiron split + skill sync), 0.18.0 (project picker + X-Actor visibility).
